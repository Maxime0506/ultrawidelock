...
static struct ultrawidelock_assert_ec_priv s_dev;
static uint8_t s_dev_pub[ULTRAWIDELOCK_ASSERT_PUB_LEN];
...
static void load_or_make_dev_key(void)
{
	...
	if (have) {
		if (ultrawidelock_ec_p256_pub_from_priv(priv, s_dev_pub) != 0) {
			return;
		}
	} else {
		if (ultrawidelock_ec_p256_keygen(priv, s_dev_pub) != 0) {
			return;
		}
...
{
	/* Idempotent (psa_crypto_init is), and ultrawidelock_reader_start() has already run
	 * it. Repeated here so the keygen below does not depend on that ordering. */
	(void)ultrawidelock_prim_init();
...
static void notify_wallet(bool present)
{
	...
	ultrawidelock_reader_notify_unlock(present);
}
...
static void fill_assert(struct ultrawidelock_assert *a,
			const uint8_t nonce[ULTRAWIDELOCK_ASSERT_NONCE_LEN],
			const uint8_t cred_id[ULTRAWIDELOCK_ASSERT_CREDID_LEN], int32_t cm,
			const struct ultrawidelock_uwb_range_integrity *ig)
{
	memset(a, 0, sizeof(*a));
	memcpy(a->nonce, nonce, ULTRAWIDELOCK_ASSERT_NONCE_LEN);
	memcpy(a->cred_id, cred_id, ULTRAWIDELOCK_ASSERT_CREDID_LEN);
...
{
	char line[2 * ULTRAWIDELOCK_ASSERT_WIRE_P256 + 32];
...
static int answer_p256(const uint8_t nonce[ULTRAWIDELOCK_ASSERT_NONCE_LEN],
		       const uint8_t cred_id[ULTRAWIDELOCK_ASSERT_CREDID_LEN], int32_t cm,
		       const struct ultrawidelock_uwb_range_integrity *ig)
{
	...
	if (!s_dev_set || ultrawidelock_assert_build_p256(ultrawidelock_assert_ec_sign, &s_dev, &a, wire,
						  sizeof(wire), NULL) != 0) {
...
static int acquire_fresh(uint8_t expected_id[ULTRAWIDELOCK_ASSERT_CREDID_LEN],
			 ...
			 bool report)
{
	uint8_t expected_pub[ULTRAWIDELOCK_ASSERT_PUB_LEN];
	uint8_t actual_pub[ULTRAWIDELOCK_ASSERT_PUB_LEN];
	uint8_t actual_id[ULTRAWIDELOCK_ASSERT_CREDID_LEN];
	...
	notify_wallet(false);
	uint32_t request = ultrawidelock_reader_presence_restart();
	...
	while (before_deadline(deadline_ms)) {
		if (ultrawidelock_reader_presence_authenticated_after(auth_checkpoint, actual_pub)) {
			ultrawidelock_assert_cred_id(actual_pub, actual_id);
...
static int prove(const uint8_t nonce[ULTRAWIDELOCK_ASSERT_NONCE_LEN])
{
...
{
	uint8_t nonce[ULTRAWIDELOCK_ASSERT_NONCE_LEN];
	...
	if (strcmp(argv[1], "credential") == 0) {
		uint8_t cred_pub[ULTRAWIDELOCK_ASSERT_PUB_LEN];
		uint8_t cred_id[ULTRAWIDELOCK_ASSERT_CREDID_LEN];

		if (!ultrawidelock_reader_presence_expected_credential(cred_pub)) {
			printf("PRESENCE-ERR expected exactly one provisioned credential\n");
			return 1;
		}
		ultrawidelock_assert_cred_id(cred_pub, cred_id);
