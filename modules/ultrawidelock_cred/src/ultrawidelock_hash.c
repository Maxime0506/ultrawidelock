...
static void sha256_block(uint32_t h[8], const uint8_t p[64])
{
	 ...
	 * through s->buf instead. The objectIndex suppression for that lives in
	 * tests/tooling/cppcheck_gate.sh, scoped to this file, because the report
	 * lands on both lines of the statement below and the marker that covers a
	 * range is newer than the cppcheck some distributions ship. */
...
void ultrawidelock_sha256_init(struct ultrawidelock_sha256 *s)
{
	s->h[0] = 0x6a09e667;
	s->h[1] = 0xbb67ae85;
	s->h[2] = 0x3c6ef372;
...
void ultrawidelock_sha256_update(struct ultrawidelock_sha256 *s, const void *data, size_t len)
{
	const uint8_t *p = (const uint8_t *)data;

	s->total += len;
	if (s->buflen) {
		size_t n = ULTRAWIDELOCK_SHA256_BLOCK - s->buflen;
		...
		p += n;
		len -= n;
		if (s->buflen == ULTRAWIDELOCK_SHA256_BLOCK) {
			sha256_block(s->h, s->buf);
			s->buflen = 0;
		}
	}
	while (len >= ULTRAWIDELOCK_SHA256_BLOCK) {
		sha256_block(s->h, p);
		p += ULTRAWIDELOCK_SHA256_BLOCK;
		len -= ULTRAWIDELOCK_SHA256_BLOCK;
	}
	if (len) {
		memcpy(s->buf, p, len);
		s->buflen = len;
...
// digest.
void ultrawidelock_sha256_final(struct ultrawidelock_sha256 *s,
				uint8_t out[ULTRAWIDELOCK_SHA256_LEN])
{
	uint64_t bits = s->total * 8;
	uint8_t pad = 0x80;

	ultrawidelock_sha256_update(s, &pad, 1);
	pad = 0;
	while (s->buflen != 56) {
		ultrawidelock_sha256_update(s, &pad, 1);
	}
	...
	ultrawidelock_sha256_update(s, lenbe, 8);
...
void ultrawidelock_sha256(const void *data, size_t len, uint8_t out[ULTRAWIDELOCK_SHA256_LEN])
{
	struct ultrawidelock_sha256 s;

	ultrawidelock_sha256_init(&s);
	ultrawidelock_sha256_update(&s, data, len);
	ultrawidelock_sha256_final(&s, out);
}

void ultrawidelock_hmac_sha256(const uint8_t *key, size_t key_len, const void *msg, size_t msg_len,
		       uint8_t out[ULTRAWIDELOCK_SHA256_LEN])
{
	uint8_t k[ULTRAWIDELOCK_SHA256_BLOCK];
	uint8_t ipad[ULTRAWIDELOCK_SHA256_BLOCK], opad[ULTRAWIDELOCK_SHA256_BLOCK];
	uint8_t inner[ULTRAWIDELOCK_SHA256_LEN];
	struct ultrawidelock_sha256 s;

	memset(k, 0, sizeof(k));
	if (key_len > ULTRAWIDELOCK_SHA256_BLOCK) {
		ultrawidelock_sha256(key, key_len, k);
	} else if (key_len) {
		memcpy(k, key, key_len);
	}
	for (size_t i = 0; i < ULTRAWIDELOCK_SHA256_BLOCK; i++) {
		ipad[i] = k[i] ^ 0x36;
		opad[i] = k[i] ^ 0x5c;
	}
	ultrawidelock_sha256_init(&s);
	/* The loop above writes all BLOCK bytes of both pads. */
...
void ultrawidelock_hkdf_extract(const uint8_t *salt, size_t salt_len, const uint8_t *ikm,
				size_t ikm_len, uint8_t prk[ULTRAWIDELOCK_SHA256_LEN])
{
	uint8_t zero[ULTRAWIDELOCK_SHA256_LEN] = {0};

	if (salt == NULL || salt_len == 0) {
		salt = zero;
		salt_len = sizeof(zero);
	}
	ultrawidelock_hmac_sha256(salt, salt_len, ikm, ikm_len, prk);
}
...
// ultrawidelock_hkdf_extract. info: context/application-specific info bytes, may be NULL if
// info_len is 0. info_len: length of info in bytes. out: buffer that receives out_len bytes of
// derived key material. out_len: number of bytes to produce; must not exceed 255 * 32 bytes (255 *
// ULTRAWIDELOCK_SHA256_LEN). Returns 0 on success, or -1 if out_len exceeds the RFC 5869 maximum.
int ultrawidelock_hkdf_expand(const uint8_t prk[ULTRAWIDELOCK_SHA256_LEN], const uint8_t *info,
			      size_t info_len, uint8_t *out, size_t out_len)
{
	...
	if (out_len > (size_t)255u * ULTRAWIDELOCK_SHA256_LEN) {
		return -1;
	}
	while (done < out_len) {
		struct ultrawidelock_sha256 s;
		...
		for (size_t i = 0; i < ULTRAWIDELOCK_SHA256_BLOCK; i++) {
			uint8_t kb = i < ULTRAWIDELOCK_SHA256_LEN ? prk[i] : 0;
		...
		tlen = ULTRAWIDELOCK_SHA256_LEN;

		size_t n = out_len - done;

		if (n > ULTRAWIDELOCK_SHA256_LEN) {
			n = ULTRAWIDELOCK_SHA256_LEN;
		}
		memcpy(out + done, t, n);
		done += n;
		counter++;
...
// out_len: number of bytes to produce; must not exceed 255 * 32 bytes, per
// ultrawidelock_hkdf_expand. Returns 0 on success, or -1 if out_len exceeds the RFC 5869 maximum.
int ultrawidelock_hkdf(const uint8_t *salt, size_t salt_len, const uint8_t *ikm, size_t ikm_len,
	       const uint8_t *info, size_t info_len, uint8_t *out, size_t out_len)
{
	uint8_t prk[ULTRAWIDELOCK_SHA256_LEN];

	ultrawidelock_hkdf_extract(salt, salt_len, ikm, ikm_len, prk);
	return ultrawidelock_hkdf_expand(prk, info, info_len, out, out_len);
}

int ultrawidelock_x963_kdf(const uint8_t *z, size_t z_len, const uint8_t *info, size_t info_len,
		   uint8_t *out, size_t out_len)
{
	uint32_t counter = 1;
	size_t done = 0;

	if (out_len > 0xffffffffu - ULTRAWIDELOCK_SHA256_LEN) {
		return -1;
	}
	while (done < out_len) {
		struct ultrawidelock_sha256 s;
		...
		ultrawidelock_sha256_init(&s);
		ultrawidelock_sha256_update(&s, z, z_len);
		ultrawidelock_sha256_update(&s, cbe, sizeof(cbe));
		if (info && info_len) {
			ultrawidelock_sha256_update(&s, info, info_len);
		}
		ultrawidelock_sha256_final(&s, digest);
